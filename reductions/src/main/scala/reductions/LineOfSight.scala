...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    output.update(0, 0)
    (1 until input.length).foreach(i => output.update(i, max(output(i-1), input(i)/i)))
  }
...
//    var maxAngle = 0f
//    (from until until).foreach(i => maxAngle = Math.max(maxAngle, if(i == 0) 0 else input(i)/ i))
//    maxAngle
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    (from until until).foreach(i =>
      if(i == 0) output.update(i, startingAngle)
      else if(i == from) output.update(from, max(startingAngle, input(i)/i))
      else output.update(i, max(output(i-1), input(i)/i)))
  }
