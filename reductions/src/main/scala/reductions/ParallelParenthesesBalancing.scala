...
object ParallelParenthesesBalancing {
  ...
  def balance(chars: Array[Char]): Boolean = {
    def recur(chars: Array[Char], openCount: Int): Int = {
      if (chars.isEmpty || openCount < 0) openCount
      else if (chars.head == '(') recur(chars.tail, openCount + 1)
      else if (chars.head == ')') recur(chars.tail, openCount - 1)
      else recur(chars.tail, openCount)
    }
  ...
  def parBalance(chars: Array[Char], threshold: Int): Boolean = {
    ...
    def traverse(idx: Int, until: Int): (Int, Int) = {
      ...
      (idx until until).foreach(i => {
        if(chars(i) == '(') openCount = openCount + 1
        else if(chars(i) == ')')
          if (openCount > 0) openCount = openCount - 1
          else closeCount = closeCount + 1
      })
      (openCount, closeCount)
    }

    def reduce(from: Int, until: Int): (Int, Int) = {
      if ((until - from) <= threshold || from < 0) traverse(from, until)
      else {
        val mid = (from + until) / 2
        val (left, right) = parallel(reduce(from, mid), reduce(mid, until))

        if (left._1 > right._2) (right._1 + left._1 - right._2, left._2)
        else (right._1, left._2 + right._2 - left._1)
      }
    }
